...
#define FILE_PATH "/spiffs/myfile.bin"
...
static esp_err_t http_event_handler1(esp_http_client_event_t *evt)
{
	 static FILE* file = NULL;
	    ...
	    switch (evt->event_id)
	    {
	        case HTTP_EVENT_ON_DATA:
	            if (evt->data_len > 0) {
	                ...
	                while (remaining > 0) {
	                    ...
	                    size_t written = fwrite(evt->data + bytes_written, 1, chunk_size, file);
	                    if (written <= 0) {
	        ...
	        case HTTP_EVENT_ON_FINISH:
	            if (file != NULL) {
	                // Close file after download is complete
	                fclose(file);
	                file = NULL;
	                bytes_written = 0;
...
void ota_ipcb(void)
{
	 esp_http_client_config_t config1 = {
	            .url = "http://nishchith.000webhostapp.com/esp32.json",
				...
				if(new_version > FIRMWARE_VERSION)
				{
					...
					if(cJSON_IsString(file) && (file->valuestring != NULL))
					{
						...
						esp_http_client_config_t http_conf = {
							...
							.event_handler = http_event_handler1
						};
						esp_http_client_handle_t http_client = esp_http_client_init(&http_conf);
						esp_http_client_set_method(http_client, HTTP_METHOD_GET);
						esp_err_t err = esp_http_client_perform(http_client);
						if (err != ESP_OK)
						{
						    ESP_LOGE(TAG, "Failed to download file from HTTP server (%s)", esp_err_to_name(err));
						    return;
						}
						// Cleanup
						...
						ESP_LOGI(TAG, "File downloaded and stored in SPIFFS");
						FIRMWARE_VERSION = new_version;
	    		}
...
void app_main(void)
{
    ...
    */
    ...
    esp_ble_helper_init();
#endif
    advanced_ota_example_task();   // main esp32 ota
    ota_ipcb();						//ota for storing data to spiff
    esp_vfs_spiffs_register(&spiffs_conf);
         size_t total, used;
            ret = esp_spiffs_info(NULL, &total, &used);
            if (ret != ESP_OK) {
                printf("Failed to get SPIFFS filesystem information. Error: %s\n", esp_err_to_name(ret));
                return;
            }
